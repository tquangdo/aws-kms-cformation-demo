...
# ~~~~~~~~~~~ 1) S3 ~~~~~~~~~~~
aws s3 rm s3://kmsworkshop-462123133781/ --recursive
aws s3api delete-bucket --bucket kmsworkshop-462123133781
...
# ~~~~~~~~~~~ 4) Role & Policy ~~~~~~~~~~~
## ~~~~~~~~~~~ ROLE!!!
roles=$(aws iam list-roles --query 'Roles[?contains(RoleName, `KMS`)].RoleName' --output text) # replace "DTQ" <=> "<ROLE_NAME>"
...
# ~~~~~~~~~~~ 14) EC2 ~~~~~~~~~~~
## ~~~~~~~~~~~ list SG!!!
instance_id=$(aws ec2 describe-instances --filters "Name=tag:Name,Values=DTQEC2KMSDemo" --query "Reservations[].Instances[].InstanceId" --output text)
